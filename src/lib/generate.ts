...
import type { StoredStyleProfile } from "@/db/schema";

// ---------------------------------------------------------------------------
// Structured-output helper: force a single tool call and return its input.
// This is version-stable across SDK releases (no reliance on a parse helper).
// ---------------------------------------------------------------------------
...
export async function buildStyleProfile(
  ...
  return toolUseInput<StoredStyleProfile>(res, STYLE_TOOL.name);
}

// ---------------------------------------------------------------------------
// Generate one post on a topic, in the profile's voice.
// ---------------------------------------------------------------------------
function styleSystemPrompt(profile: StoredStyleProfile, niche: string): string {
  return [
    `You write X (Twitter) posts for the Cloudsheer account in a specific author's voice.`,
    `The niche is: ${niche}.`,
    ``,
    `VOICE: ${profile.voice}`,
...
export async function generatePost(args: {
  ...
  const res = await anthropic.messages.create({
    model,
    max_tokens: 400,
    // The style block is frozen per profile, so cache it: repeated generations
    // for the same profile in one run reuse the cached prefix.
    system: [
      {
        type: "text",
